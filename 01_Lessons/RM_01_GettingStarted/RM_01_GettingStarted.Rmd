...
In the following, I will briefly give a general outline of R Markdown, which is an easy to learn language for creating updatable and reproducible reports. 
<!-- You may ask why I place every sentence on a new line. You will learn in lesson five that this makes collaborative work on R Markdown documents using GitHUb much easier, and still, all text of a paragraph is displayed correctly in the final document. And by the way, you now also have learned how to insert a comment into an R Markdown document -->
...
I did this because I wanted the examples to be indented. 
Second, for the last three lines, I embedded the results in a double \$ which results in the text being interpreted as LaTeX text, hence, it will be formatted that way. 
...
In this example, to have the same set of random variables and the same correlation returned each time, I set a custom seed, which is particularly important for arriving at reproducible results when doing simulations or bootstrapping. 
Here, I set the seed to 242, my favorite number (after all powers of two), but the number actually does not matter as long as it is always the same for the same analysis[^1]. 
